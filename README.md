...
2. **Product-factorial mode** uses GCM/wGCM residual products and requires product assignment odds or explicit reweighting to a product design. Every reference GCM result carries serialized design evidence bound to a completed audit identifier and SHA-256 source fingerprint; the engine must still resolve that reference in the evidence ledger. Unverified calculations are permanently marked `diagnostic_only`.
