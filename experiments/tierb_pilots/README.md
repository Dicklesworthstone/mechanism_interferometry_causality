...
|---|---|---|
| `conservation_pilot.py` | C-001 (claimed independent-route detection), C-002 (coherent route invisible), C-003 (scalar-check kernel), mode-D construction | C-001 is **refuted as implemented**: because the script reconstructs `kappa=log(rab/(ra*rb))` from the same arrays, its signed discrepancy is exactly `E[rab]-E[ra]E[rb]`, so it is only the already-required raw-normalizer check and no out-of-family population can rescue the estimator-QA claim. C-002 survives only as same-array algebra consistency. C-003 survives with rank qualification (finite support of size m and k witness equations leave nullity `m − rank(L)`; on general spaces the honest statement is that finite batteries leave the orthogonal complement unchecked). Mode D is refuted, see below. |
