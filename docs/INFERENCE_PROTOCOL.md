...
The reference `mic_stats::gcm_projection` API cannot be called without a `ProductDesignEvidence` value, and every returned `GcmEstimate` serializes that evidence. `from_sampling_odds_audit` requires the completed audit identifier and a SHA-256 fingerprint of its declared allocation source, then recomputes the pooled log odds and refuses non-product inputs. `from_reweighting_audit` likewise requires the completed audit identifier and artifact fingerprint, not a future plan. `diagnostic_only` keeps an unverified residual-product calculation visibly ineligible for certificate use. Deserialization reruns all structural and numerical checks, so a forged `product_odds_verified` JSON object cannot acquire authority merely by bypassing the constructor. The engine still cross-checks the audit reference against the evidence ledger: a syntactically valid identifier cannot prove an audit occurred, and empirical counts that happen to have product odds remain diagnostic only.
