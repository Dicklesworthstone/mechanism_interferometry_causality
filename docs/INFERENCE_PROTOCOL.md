...
The reference `mic_stats::gcm_projection` API cannot be called without a `ProductDesignEvidence` value, and every returned `GcmEstimate` serializes that evidence. `from_corner_odds` recomputes the pooled log odds and refuses non-product inputs; `from_reweighting_audit` requires the identifier of a completed reweighting audit, not a future plan; `diagnostic_only` keeps an unverified residual-product calculation visibly ineligible for certificate use. Deserialization reruns these checks, so a forged `product_odds_verified` JSON object cannot acquire authority by bypassing the constructor. The probabilities still remain empirical claims and must agree with the manifest and preflight audit.
