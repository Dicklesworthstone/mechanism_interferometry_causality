...
| P1 | [Sachs protein-signaling interventions](https://www.bnlearn.com/research/sachs05/) | 5,400 observations across 10 environments, 11 measured signaling quantities, and recorded stimulatory/inhibitory interventions | Small many-environment dictionary and graph benchmark, intervention-label ablation, nonlinear/local-support stress | Intervention metadata are experimental; the “gold standard” network is literature-validated rather than exact physical truth, and cells are not automatically independent replicates | Hide `INT` and require authority to fall; restore it and compare held-out intervention response, but never treat 5,400 cells as 5,400 experimental units or the reference network as infallible |
...
| P2 | [Tennessee Eastman Process simulation](https://doi.org/10.7910/DVN/6C3JR1) | Large multivariate chemical-process trajectories with normal operation and 20 recorded fault types | Feedback-heavy dynamics, fault-environment factorization, controller response, timing, and fat-intervention abstention | Simulator fault labels are known; a fault may perturb several mechanisms and the process/controller graph is not identified merely by the label | Hide fault onset/type, reverse time, and separate operating-condition/controller response from fault propagation; unknown redistribution terms or license status block vendoring |
...
| P2 | [ACIC 2016 semi-synthetic challenge](https://github.com/vdorie/aciccomp) | 78 public semi-synthetic instances built from real covariates, with varied assignment and response surfaces | Effect-estimation calibration, overlap, heterogeneity, coverage, and abstention when nuisance fits disagree | Counterfactual outcomes are simulator-known; this authorizes effect scoring, not graph or mechanism truth | Preserve treatment/prevalence variation and score interval coverage plus wrong-sign rate; a low prediction loss cannot rescue a miscalibrated effect claim, and redistribution/license status must be checked before vendoring |
| P3 | [UCI HAR smartphone activities](https://archive.ics.uci.edu/dataset/240/humanactivityrecognitionusingsmartphones) | 10,299 overlapping windows from 30 subjects performing six instructed activities; 561 derived inertial features; CC BY 4.0 | Cheap Arrow/Parquet and representation adapter, subject-held-out sensor transport, overlap-unit discipline, direction abstention | Activity instructions and labels are controlled, but windowed sensor features do not identify a causal graph among bodily motion and sensors | Split only by subject, never by overlapping windows; hide activity labels and require loss of action authority; mirrored sensor/body interpretations must remain predictive-only |
...
The compact Sachs archive is the corresponding fast many-environment benchmark:
its intervention indicator can be withheld and restored, but its cells and
literature network carry weaker authority than a randomized chamber run.
...
The Tennessee Eastman simulator adds a deliberately difficult controller-loop
case in which a named fault can be a fat intervention and downstream regulation
can reverse naive lag order.
...
ACIC 2016 adds simulator-known counterfactual effects across many assignment and
response surfaces; it tests effect coverage and sign calibration, never
mechanism-family or graph recovery.
...
good.
UCI HAR is a cheap version of the same lesson for representation pipelines:
instructed activity predicts sensor traces, but overlapping windows and passive
body/sensor relations do not license an internal causal graph.
...
The next three adapters are chosen to test different reasons an arrow can be
licensed rather than three versions of the same prediction problem:

1. **Causal Chambers `lt_interventions_standard_v1`.** Freeze the official
