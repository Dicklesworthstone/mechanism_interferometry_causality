...
These facts yield a finite biconditional certificate.  Relative to a proposed directed acyclic graph and target assignment, locality, targetwise conditional normalization, and zero square curvature on every observed face are necessary and sufficient for the existence of a context-invariant modular soft-intervention representation.  A master identity,
\[
\E_0[r_e(X)\mid X_A]=\frac{p_e(X_A)}{p_0(X_A)},
\]
turns orientation into ordinary marginal invariance testing: within a localized causal family, the target is the coordinate whose deletion leaves the remaining marginal unchanged.  The number of invariant deletions is itself diagnostic: undercoverage degrades gracefully, affected descendants generically produce no pass, and balanced parity mechanisms can produce multiple passes.
...
\]
It is the conditional design log-odds ratio after subtracting the known sampling-design odds.  Full-state modularity means \(\kappa\equiv0\).  Marginalization can create curvature even when the complete mechanisms compose flatly.  We derive the exact identity
\[
e^{\kappa_{AB}(x)}-1=
...
\begin{enumerate}
\item \textbf{A finite biconditional certificate.}  For common-support soft interventions with proposed distinct targets, locality, conditional normalization, and zero curvature on every two-dimensional design face are necessary and sufficient for a modular causal representation.  Pairwise square flatness suffices; all higher Boolean-lattice curvatures then vanish automatically.

\item \textbf{Ratio-free orientation.}  The identity \(\E_0[r_e\mid X_A]=p_e(X_A)/p_0(X_A)\) converts target orientation into low-dimensional two-sample equivalence tests.  It also pins the otherwise unidentified additive constant of an unnormalized discriminative log-ratio score.

\item \textbf{Support-error diagnostics.}  The deletion rule is robust to omitted parents and unaffected nondescendants, but affected descendants generically make every deletion fail.  Zero, one, or multiple invariant deletions become model diagnostics rather than merely estimator outputs.
...
Assume that \(p_0\) factorizes over a proposed DAG \(G\), all regimes have common support, and primitive perturbations are assigned distinct proposed targets \(t_1,\ldots,t_K\).  Let
\begin{equation}
...
\end{equation}
but the intervention ratio happens not to depend detectably on one or more true parents, then deleting \(t\) still leaves a subset of parents.  The target passes, and under deletion faithfulness it remains the unique pass.  The recovered parent set is \(\widehat S\setminus\{t\}\subseteq\pa(t)\).  Undercoverage therefore misses edges rather than reversing the target.
...
\midrule
Exactly one & Coherent single-target family with identifiable orientation. \\
...
Multiple asymmetric tilts of the same target are the natural remedy.  The target must pass every valid tilt; a spurious symmetry usually does not.  Taking the union of supports across tilts also reveals parents omitted by a particular intervention.
...
For any bounded witness \(w\), define
\begin{equation}
M_w
=
...
Let the complete state be \(Z=(X,U)\), with baseline density \(q_0\).  Suppose the complete-state regime factors satisfy
\begin{equation}
q_A=q_0L_A,
\qquad
q_B=q_0L_B,
...
Let \(q_{\alpha,\beta}(z)\) be a smooth complete-state intervention family, and let
\begin{equation}
...
\begin{align}
\partial_{\alpha\beta}^2\log p_{\alpha,\beta}(x)
={}&
\E\left[\partial_{\alpha\beta}^2\log q_{\alpha,\beta}(Z)\mid X=x\right]
\notag\\
&+
\Cov(S_A,S_B\mid X=x).
...
Equation \eqref{eq:nested-state} holds whenever the complete-state factors satisfy \eqref{eq:latent-flat}.  If \(W\) supplies the missing separator so that \(\kappa_{XW}=0\), then
\begin{equation}
r_A^Xr_B^X(e^{\kappa_X}-1)
=
\Cov(r_A^{XW},r_B^{XW}\mid X).
...
\end{equation}
not \(\kappa\equiv0\), unless \(\OR(\rho)=1\).  Equal-corner balancing, \(\rho_{ab}=1/4\), is safe because it is product sampling.  Arbitrary per-corner quotas are not.  The implementation must either enforce product sampling, reweight observations to a chosen product design, or use Track I.
...
A concrete localization procedure treats the regime label as the response in Markov-boundary discovery.  A forward phase repeatedly adds the state variable with the largest reproducible cross-fitted improvement in proper regime-prediction loss, which at the Bayes optimum equals conditional mutual information.  A backward phase removes variables whose conditional contribution disappears.  This is an intervention-adapted form of IAMB \citep{tsamardinos2003iamb}.
...
An ensemble alternative to the greedy path exploits the parsimony corollary directly.  Sample many random candidate supports, fit a regime classifier on each, and score each support by held-out proper regime-prediction loss together with an explicit complexity measure.  Retain the parsimony frontier: the supports within a fixed tolerance of the best held-out loss, ordered by complexity.  Per-variable inclusion frequencies across the frontier are then reported as localization stability paths in the audit bundle.  By locality, the true support \(\{t\}\cup\pa(t)\) is the smallest support carrying full regime information, so the frontier filter has a causal justification here that generic model selection lacks.  Two disciplines keep the procedure honest: the frontier threshold is computed once over the completed ensemble, never incrementally as results accumulate, and inclusion frequencies are normalized per variable so that they remain interpretable as marginal stability estimates.  The deletion pass-count audit then independently checks the localized support.

\subsection{Orientation requires equivalence inference}

Failure to reject equality is not evidence for invariance.  Choose a nonnegative marginal discrepancy \(D\), such as MMD \citep{gretton2012mmd} or energy distance \citep{szekely2013energy}.  For each deletion,
\begin{equation}
...
No individual probability identity in this paper should be claimed as novel.  The narrow contribution is the assembled object:
\begin{equation}
\boxed{
...
Mechanism interferometry treats a factorial family of empirical regimes as one constrained mathematical object.  The central logic can be summarized in four lines:
\begin{align}
\text{locality}
&\Longrightarrow
...
\item moment battery, GCM/wGCM projections, and joint-model diagnostics;
