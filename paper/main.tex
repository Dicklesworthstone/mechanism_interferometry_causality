...
The inferential architecture has two complementary tracks.  With arbitrary regime-conditioned samples and externally justified state-independent sampling proportions, four-law functionals \(M_w\) identify weighted curvature without a product-assignment model.  Under verified product factorial randomization, residual-product scores test projected curvature.  Localization uses the same cross-fitted nuisance discipline; orientation uses simultaneous equivalence tests; and held-out combination prediction preserves the raw compositional normalizer.  The released implementation provides exact algebra and finite-state completion oracles plus explicitly uncalibrated cluster-cross-fitted diagnostics.  Calibrated closure inference and a controlled-real factorial result remain necessary before empirical certification claims.
\end{abstract}
...
\begin{enumerate}
\item \textbf{Complete and partial modularity characterizations.}  For complete common-support factorial families with proposed distinct targets, locality, conditional normalization, and zero curvature on every face are necessary and sufficient for a modular representation.  The categorical theorem permits arbitrary alternative levels within one family.  For strictly positive finite-state laws relative to a fixed DAG and target map, the partial-design kernel theorem separates kernel identification from background-reuse testability even when the main-effects matrix is rank deficient.

\item \textbf{Orientation with explicit failure states.}  Under separately justified single-target semantics and deletion faithfulness, \(\E_0[r_e\mid X_A]=p_e(X_A)/p_0(X_A)\) converts orientation into equivalence tests.  Zero, one, multiple, or statistically undetermined deletion passes remain distinct outcomes; support errors, weak shifts, and symmetry produce abstention rather than forced targets.

\item \textbf{Curvature, missing information, and state completion.}  We identify curvature with excess conditional design log odds, derive conservation identities connecting complete and observed states, and show that infinitesimal observed curvature is a conditional covariance of complete-state intervention scores.  These identities motivate held-out, common-cohort measurement-block search without claiming that generic finite curvature is positive semidefinite or low rank.

\item \textbf{Auditable inference and empirical protocol.}  We keep arbitrary-quota four-law moments separate from product-design residual scores; require overlap, unit, selection, and raw-normalizer reporting; and specify cluster-split witnesses, held-out combination prediction, feature-flag validation, and Perturb-seq analysis.  The release implements exact oracles and diagnostic fitted slices.  Cluster-resampled calibrated closure inference, autonomous dictionary learning, and a controlled-real result remain targets rather than claimed achievements.
\end{enumerate}
...
The reference implementation accepts cluster-level contributions for a witness
family frozen before confirmation and evaluates the symmetrized moment
\begin{equation}
\widehat M_w=\widehat\E_{AB}[w]
-\frac12\left\{\widehat\E_A[w\widehat r_B]
+\widehat\E_B[w\widehat r_A]\right\}.
\end{equation}
It centers each law contribution separately, accumulates the signed terms into
one influence vector per declared dependence unit across all three law strata,
and applies one Rademacher sign to that joint unit.  Thus a crossover unit seen
under several arms is never resampled as several independent units.  A max
statistic across the frozen witnesses retains the declared unit, seed, and
witness-family fingerprint. This is an implemented reference
asymptotic interval procedure, not yet certificate-grade calibration: the
current artifact explicitly records that finite-sample coverage has not been
validated across the cluster-size, overlap, and nuisance-misspecification
gauntlet. Adaptive witness learning still requires an outer discovery split.
...
\end{equation}
Report \(\widehat Z_T-1\) rather than silently discarding it: it is the empirical residual of a necessary moment restriction.  Only calibrated bounds excluding a declared equivalence region license calling that restriction failed.  Compare \(\widetilde P_T\) with held-out samples from \(P_T\) using proper scores, MMD, energy distance, or prespecified transported moments.

The reference fitted implementation enforces this separation in two stages.  The first stage accepts only \(00,10,01\), fits three-class regime classifiers under a declared cluster-weighted pooling law, and constructs fold-indexed out-of-fold baseline values
\[
  \widehat g_{-f}(x)
  = \widehat{\log r_{A,-f}}(x)+\widehat{\log r_{B,-f}}(x)-\log\widehat Z .
\]
The API can then accept only a separately supplied \(11\) sample under the same declared unit and feature-transform fingerprints; the reference CLI freezes stage one before it opens the stage-two file.  This proves that \(11\) rows were not passed to the nuisance-fitting API, but it does not prove the historical claim that an upstream analyst chose the feature transform without inspecting \(11\).  That stronger isolation claim requires a trusted execution receipt and remains false in the diagnostic contract by default.  A descriptive score, computable in \(O(n_0+F n_{11})\) time for \(F\) folds, is the equal-prior density-ratio log loss
\begin{equation}
  \widehat L
  = \frac12\sum_f\omega_f\widehat\E_{0,f}\!\left[\log(1+e^{\widehat g_{-f}(X)})\right]
  + \frac12\sum_f\omega_f\widehat\E_{11}\!\left[\log(1+e^{-\widehat g_{-f}(X)})\right],
  \label{eq:heldout-ratio-score}
\end{equation}
where \(\omega_f\) is the baseline-unit mass in fold \(f\).  The same frozen fold mixture is used in both class expectations, with equal mass per declared independent unit; \(\log 2-\widehat L\) is reported as improvement over the uninformative ratio-one prediction.  This is a proper loss for the frozen-feature fold mixture, not by itself a claim of raw-state agreement.  For modest samples the implementation also reports the exact weighted empirical energy distance under a frozen Euclidean feature metric, rejecting distances outside finite floating-point range.  Neither number is a calibrated equivalence test.  More importantly, neither may replace the other component of the closure claim: \(\widehat Z\approx1\) alone does not imply agreement with \(P_{11}\), while a self-normalized predicted law can agree perfectly with \(P_{11}\) even when \(\widehat Z\ne1\).  Any resampling-based uncertainty procedure used here must resample the declared independent units and, absent a theorem justifying frozen-nuisance inference, refit the complete primitive nuisance pipeline.  An ordinary row permutation is not justified, and the present subset ensemble is descriptive rather than such a calibrated procedure.

The software therefore also provides deterministic, arm-stratified cluster subsampling of the transport nuisance and scoring stages, conditional on the frozen feature transform.  Each selected unit is retained intact and at most once in a refit; all primitive nuisance models and the global normalizer are recomputed, while combination units are sampled and scored only in stage two.  The seed, complete resample-plan fingerprint, realized retained count in every arm, metric-specific attempted and successful plan diversity, per-arm inclusion coverage and frequency range, and every failed-refit reason are retained.  Central empirical quantiles of \(\widehat Z-1\) and cluster ESS are reported only when the successful primitive subset ensemble varies; score and energy quantiles require variation among successful joint primitive--combination plans and a declared completion-rate floor.  Energy quantiles are further suppressed unless every completed score satisfied the exact-distance work budget.  The order-statistic convention is serialized.  These seed-specific subset-ensemble summaries are deliberately not labeled confidence intervals or general robustness guarantees: without a theorem and coverage validation for the exact clustered, cross-fitted statistic, they provide no calibrated rejection or equivalence authority.
...
Classify a deletion as certified invariant if its upper bound is below \(\eps\), certified changed if its lower bound exceeds \(\eps\), and otherwise undetermined.  Exactly one invariant deletion with every competitor certified changed is a \emph{unique pass pattern}, not by itself a target.  A separate premise-bearing route may orient only after externally establishing the single-target semantics and deletion-faithfulness assumptions.  If \(D_{\mathrm{full}}\) is too small, abstain: an intervention too weak to detect cannot support that route reliably.
...
A locally prediction-locked GHCN prototype illustrates both the opportunity and the
...
For finite positive laws, the fixed-model fiber admits a more direct kernel
characterization.

\begin{theorem}[Finite partial-design kernel completion]
\label{thm:partial-kernel-completion}
Fix a DAG \(G\) on a finite Cartesian state space, a strictly positive supplied
baseline \(p_0\) that factorizes over \(G\), and categorical families with
pairwise distinct fixed targets.  Let \(D\) index strictly positive observed
regime laws, and let
\(k_v^{(s)}(x_v\mid x_{\pa(v)})\) be the unique conditional kernel of node \(v\)
under regime \(s\in D\).  The observed laws admit a modular completion if and
only if:
\begin{enumerate}
\item every \(p_s\) factorizes over \(G\);
\item every node not targeted by an active family in \(s\) retains its baseline
kernel; and
\item whenever two regimes carry the same nonbaseline level \(a\) of family
\(j\), their kernels for the target \(t_j\) agree.
\end{enumerate}
For the named query consisting of the complete nonparametric family-level
kernel dictionary and all laws on the categorical grid, and assuming every
target has at least two states with no parametric tying across levels, the
completion is point identified exactly when every nonbaseline family-level
appears in at least one observed regime.  Otherwise it is set identified.
\end{theorem}

\begin{proof}
Necessity follows directly from the modular product: inactive factors cancel,
and a replacement kernel depends only on its family and level, not on the
background.  Conversely, factorization and inactive-kernel equality reduce
every observed law to its active target kernels.  Repeated-level equality makes
each observed replacement kernel well defined.  Insert those kernels and choose
arbitrary strictly positive normalized kernels for absent levels; the resulting
DAG products normalize and reproduce every observed regime.  Unique
factorization of a positive DAG law identifies every observed target kernel.  If
all nonbaseline levels appear, this identifies the entire dictionary and hence
every grid law.  If a level is absent, two distinct positive normalized kernels
for its nontrivial target produce two distinct completions.
\end{proof}

This identification statement is independent of overidentifying testability: a
level observed once is identified under the fixed model but its background
reuse is untested, while repeated appearances test kernel invariance directly.
The supplied baseline, strict positivity on every declared parent context,
distinct targets, and fixed graph are load-bearing.  Without a supplied
baseline, every level zero must instead be covered somewhere; without common
parent support, kernels are identified only almost everywhere and cannot be
transported to newly supported contexts.  The result identifies neither the
physical target labels nor the graph from rows and does not survive
state-dependent selection without a valid recovery model.
...
\end{equation}
A fixed-primitive model relative to an arbitrary density-ratio reference has
\begin{equation}
H_e(x)=b(x)+\sum_{j=1}^K A_{ej}\phi_j(x),
\qquad r_j(x)=\exp\{\phi_j(x)\}.
\label{eq:environment-factorization}
\end{equation}
Here \(b\) is the transport from the density-ratio reference to the factorial
zero law.  It vanishes only when those two laws coincide.
...
\eqref{eq:environment-factorization} is known and the augmented matrix
\([\mathbf 1,A]\) has column rank \(K+1\), then \(b\) and the function-valued
factor vector \((\phi_1,\ldots,\phi_K)\) are unique almost everywhere.  When
the density-ratio reference is the marked zero-code law, \(b=0\) and column
rank \(K\) of \(A\) suffices.  If instead the environment labels are unknown
but all \(2^K\)
vertices
\begin{equation}
...
and binary codes are identified up to coordinate permutation and independent
coordinate bit complementation.
...
The first claim follows by applying a left inverse of \([\mathbf 1,A]\)
pointwise, or of \(A\) after subtracting a marked zero-code row.  For the
...
A marked pure-environment design is a useful corollary of the first claim.  If
the factorial baseline \(e_0\) and one environment with code \(e_j\) are
externally justified as changing only primitive \(j\), then
\(\phi_j=H_{e_j}-H_{e_0}\) directly.  When the density-ratio reference is that
factorial baseline, \(H_{e_0}=0\).  Linear independence of the recovered
functions makes the real coefficient vector of every other environment unique
whenever it lies in their span.  This is an algebraic dictionary result, not
evidence that the columns are causal mechanisms.  If a physical anchor
amplitude in \(H_{e_j}-H_{e_0}=\lambda_j\phi_j\) is unknown, the data identify
only its factor ray; taking the observed anchor difference as a unit potential
fixes scale by convention.  Arbitrary rescaling generally fails conditional
normalization and cannot silently be called another replacement.
Without known codes, pure anchors, known single-change adjacency, or the
complete parallelotope, even exact sparse-looking factorizations can remain
ambiguous.  Choose linearly independent observed transports \(h_1,h_2\).  Both
\begin{equation}
  (h_1,h_2),\quad A=\begin{bmatrix}1&0\\0&1\end{bmatrix},
  \qquad\text{and}\qquad
  (h_1,h_2-h_1),\quad A'=\begin{bmatrix}1&0\\1&1\end{bmatrix}
\end{equation}
reproduce the same three rows \((0,h_1,h_2)\) exactly but predict different
missing fourth laws.  More generally \(A\Phi=(AT)(T^{-1}\Phi)\)
for every invertible \(T\).  Sparsity can score representatives of this orbit,
but it does not by itself prove which vocabulary generated the environments.
Repeated backgrounds become identifying only under an explicit uniqueness
theorem or independently supplied primitive adjacency; selecting apparently
parallel edges and validating them on the same laws is circular.
...
now also ship a deterministic linear reference joint-regime model and
dependence-unit outer cross-fitting.  Assignment episodes may carry different
corners only while nested inside one dependence unit, and the declared corner
mass is divided equally by dependence unit before episode and row subdivision.
out-of-fold baseline moments of its regularized linear interaction projection, and an exact
finite-state leave-the-combination-arm-out predictor that preserves the raw
normalizer and evaluates the untouched law by total variation and Hellinger
distance.  A full-rank finite-state reference solver identifies factor-level
log potentials relative to a fixed DAG and target map, then checks Markov
factorization, locality, and conditional normalization.  A second exact
finite-state solver works directly in fixed-DAG conditional kernels and handles
arbitrary partial grids under the positivity and supplied-baseline conditions
of Theorem~\ref{thm:partial-kernel-completion}.  These are diagnostic
reference paths: the software does not yet ship
a calibrated global full-law closure test for flexible learners, adaptive
witness confirmation, nonparametric localization, mechanism-dictionary
identification from unknown environment codes, or the Python/Arrow interfaces
needed for million-row scientific tables.  The
...
\begin{itemize}[nosep, leftmargin=1.5em, font=\ttfamily]
  ...
  \item incomplete\_common\_support
  \item duplicate\_provenance
\end{itemize}

Informational findings carry a separate declared set:

\begin{itemize}[nosep, leftmargin=1.5em, font=\ttfamily]
  \item estimator\_family\_agreement
  \item histogram\_not\_a\_certificate
  \item histogram\_projection
  \item overlap\_adequate
  \item orientation\_unique\_target
\end{itemize}  The split is deliberate rather than
