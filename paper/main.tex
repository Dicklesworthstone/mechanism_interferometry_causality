...
\end{equation}
not \(\kappa\equiv0\), unless \(\OR(\rho)=1\).  Equal-corner balancing, \(\rho_{ab}=1/4\), is safe because it is product sampling.  Arbitrary per-corner quotas are not.  The implementation must either enforce product sampling, reweight observations to a chosen product design, or use Track I.  Accordingly, every software GCM projection carries serialized design evidence: directly verified corner odds, a reference to a completed product-design reweighting audit, or an explicit diagnostic-only grade that cannot support certification.
