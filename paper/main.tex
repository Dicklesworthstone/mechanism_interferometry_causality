...
\item \textbf{An auditable empirical and software protocol.}  We specify localization, orientation, curvature testing, overlap gates, compositional prediction, state expansion, simulations, engineered feature-flag validation, Perturb-seq analysis, and a safe-Rust implementation architecture.  The current release implements the exact algebra, design audits, fail-closed gates, proposal boundaries, cluster-weighted joint-model cross-fitting, and a two-stage fitted compositional-prediction diagnostic.  Cluster-resampled calibrated closure inference and the autonomous mechanism-dictionary loop remain research and implementation targets rather than claimed results.
\end{enumerate}
...
now also ship a deterministic linear reference joint-regime model, corner-stratified
outer cross-fitting with equal unit weight within corner under the declared pooling law,
out-of-fold baseline curvature moments, and an exact
...
distance.  A separate full-rank finite-state reference solver identifies
factor-level log potentials relative to a fixed DAG and target map, then checks
Markov factorization, locality, and conditional normalization; rank-deficient
nonlinear fibers remain conservatively unresolved.  These are diagnostic
